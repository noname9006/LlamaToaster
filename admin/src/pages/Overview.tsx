...
import type { AdminStats, AdminTestSummary, AppSettings, TestStatus } from "../types";
import { SignIn } from "../components/SignIn";
// The main site's own status pill and id shortener, so a test reads the same
// in this table as it does on its owner's Tests page.
import { TestStatusPill } from "../../../client/src/components/StatusPill";
import { shortId } from "../../../client/src/utils";
...
// Multi-user Stage 5 (MULTIUSER_PLAN.md §5.1): a stats row, platform settings,
// the filterable global tests table, an export button -- no chat panel and no
// test-triggering UI. Every row opens that test's own page (pages/TestPage.tsx),
// which renders it the way its owner sees it, read-only. This page is the
// landing screen; the router lives in App.tsx.
export function Overview() {
  ...
  // Keeps the table current while the console is open: a test that starts,
  // progresses or finishes shows up without a reload, so any of them can be
  // opened live. Skipped while the tab is hidden, and a failed refresh keeps
  // the last good table rather than blanking it.
  useEffect(() => {
  ...
  if (phase === "unauthorized") return <SignIn />;
      ...
      <section className="mt-8">
        ...
        <div className="mt-3 overflow-x-auto rounded-lg border border-border">
          <table className="w-full min-w-[720px] text-left text-sm">
            ...
            <tbody>
              {filteredTests.map((r) => (
                <tr key={r.id} className="border-t border-border hover:bg-surface-raised/50">
                  <td className="px-3 py-2">
                    <Link to={`/tests/${r.id}`} className="text-accent hover:underline" title="Observe this test">
                  ...
                  <td className="px-3 py-2 text-muted">
                    <Link to={`/tests/${r.id}`} className="hover:text-accent hover:underline">
                      {r.modelFilename ?? r.modelId}
                    </Link>
                  </td>
                  <td className="px-3 py-2 text-muted">{r.llamaCppBackend}</td>
                  <td className="px-3 py-2">
                    <TestStatusPill status={r.status as TestStatus} />
                  </td>
                  <td className="px-3 py-2 text-muted">
