...
describe("POST /api/runs/:id/probe-result (N2)", () => {
  ...
  it("stores every measurement a rung reports, dropping none silently", async () => {
    ...
    const rung = {
      ...
      // Signed: a clean load's normal reading, which must survive validation.
      gpu_in_system_ram_mib: -15.76,
      gpu_spill_jitter_mib: 4,
      host_backed_fail: "cache",
      spill_method: "growth",
      spill_shared_growth_mib: 20,
      // Signed too: a buffer moving into VRAM drives d below zero.
      spill_unlanded_growth_mib: -252,
  ...
  it("carries speeds and slope to a reusing run, not just the pass/fail", async () => {
    ...
    await post(
      "/api/runs/dedup-full-root/probe-attempt",
      {
        ...
        gpu_buffers_mib: 3634.7,
        gpu_in_system_ram_mib: 828.43,
        gpu_spill_jitter_mib: 12,
        host_backed_fail: "layers",
        spill_method: "growth",
        spill_shared_growth_mib: 145,
        spill_unlanded_growth_mib: 130,
