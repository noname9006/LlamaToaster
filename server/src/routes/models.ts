...
export async function modelsRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.post<{ Params: { id: string } }>(
    ...
    async (request, reply) => {
      ...
      // Both must already be known to skip the lookup -- an earlier version of
      // this check only looked at n_layer, which meant a model that already
      // had n_layer (true for every model registered before mtp_layers
      // detection existed) could never get mtp_layers backfilled at all, since
      // the request never got this far.
      ...
      const authed = resolveAuthUser(request);
      const meta = repo.workerRepo.findModelFileMeta(authed?.user.id, filename) ?? { n_layer: null, mtp_layers: null };
      ...
      // meaningless 0" posture the download route already uses. Also
