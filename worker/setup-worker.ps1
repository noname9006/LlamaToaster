...
function Install-ToasterShim {
    ...
    $lines = @(
        ...
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1"',
        'goto :eof',
        ':update',
        'rem Pull the latest code, refresh dependencies, then start -- the same',
        'rem thing re-running the install command does, without re-fetching the',
        'rem bootstrap script.',
        'pushd "%TOASTER_HOME%"',
        'git fetch origin main && git reset --hard FETCH_HEAD || echo Update skipped -- this install is not a git checkout.',
        'call npm install --ignore-scripts',
        'popd',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1"',
        'goto :eof',
        ':reconnect',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1" -Reconnect',
        'goto :eof',
        ':logs',
        ...
        'echo   toaster update      pull latest code + dependencies, then start',
