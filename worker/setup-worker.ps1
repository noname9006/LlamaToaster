...
# Registers a "toaster" command for the CURRENT USER so every later
# start/update/restart is one word from any folder, instead of remembering
# where the checkout is and which of setup-worker.ps1 / start.bat to run.
#
# Deliberately minimal footprint, because a global command is a bigger ask
# than "run a script in a folder": one .cmd file under %LOCALAPPDATA%, and
# one entry appended to the USER Path environment variable. Nothing in
# Program Files, nothing in HKLM, no admin prompt, no service, no scheduled
# task. `toaster uninstall` reverses exactly those two things, and this
# prints both paths so the change is visible rather than implied.
#
# The checkout path is baked into the generated .cmd rather than looked up at
# run time, and the file is regenerated on every setup run -- so moving the
# install and re-running setup re-points it, with no stale pointer file to go
# out of sync.
function Install-ToasterShim {
    param([string]$RepoRoot)

    $binDir = Join-Path $env:LOCALAPPDATA "LlamaToaster\bin"
    $shimPath = Join-Path $binDir "toaster.cmd"
    New-Item -ItemType Directory -Force -Path $binDir | Out-Null

    # Written as .cmd, not .ps1: a .cmd is runnable from cmd.exe, PowerShell
    # and the Run dialog alike, and -- unlike a .ps1 -- is not subject to the
    # execution policy bootstrap.ps1 has to work around. It shells into
    # PowerShell itself for the steps that need it.
    $lines = @(
        '@echo off',
        'rem Generated by worker\setup-worker.ps1 -- do not edit, it is rewritten',
        'rem on every setup run. Remove with: toaster uninstall',
        "set ""TOASTER_HOME=$RepoRoot""",
        ...
        ':start',
        'rem Plain start. setup-worker.ps1 is idempotent once config.json',
        'rem exists -- it skips setup entirely and just runs the worker.',
        'rem "& exit /b" on the same line: setup-worker.ps1 rewrites THIS file',
        'rem while it runs, and cmd reads batch files by byte offset -- ending',
        'rem on an already-parsed line means it never reads the new copy.',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1" & exit /b',
        ':update',
        'rem Pull the latest code and refresh dependencies, then start -- but only',
        'rem if that succeeded. update-worker.ps1 prints the real reason on failure;',
        'rem starting anyway would silently run the old code.',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\update-worker.ps1"',
        'if errorlevel 1 exit /b 1',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1" & exit /b',
        ':reconnect',
        'powershell -NoProfile -ExecutionPolicy Bypass -File "%TOASTER_HOME%\worker\setup-worker.ps1" -Reconnect & exit /b',
        ':logs',
        'explorer "%TOASTER_HOME%\worker\logs"',
        'goto :eof',
        ':where',
        'echo %TOASTER_HOME%',
        'goto :eof',
        ':uninstall',
        'powershell -NoProfile -ExecutionPolicy Bypass -Command "$b=Join-Path $env:LOCALAPPDATA ''LlamaToaster\bin''; $p=[Environment]::GetEnvironmentVariable(''Path'',''User''); $kept=@(($p -split '';'') -ne $b) -ne ''''; [Environment]::SetEnvironmentVariable(''Path'', ($kept -join '';''), ''User''); Write-Host (''Removed '' + $b + '' from your user PATH.'')"',
        'del /q "%LOCALAPPDATA%\LlamaToaster\bin\toaster.cmd"',
        'echo Removed the toaster command. Your LlamaToaster folder, config and models were left alone.',
        'goto :eof',
        ':unknown',
        'echo Unknown command: %~1',
        ':help',
        'echo   toaster             start the worker (same as: toaster start)',
        'echo   toaster update      pull latest code + dependencies, then start (stops on any error)',
        'echo   toaster restart     start again after stopping with Ctrl+C',
        'echo   toaster reconnect   re-approve this machine after its session was revoked',
        'echo   toaster logs        open the worker log folder',
        'echo   toaster where       print the install folder',
        'echo   toaster uninstall   remove this command (leaves the install alone)',
        'echo.',
        'echo Stop a running worker with Ctrl+C in its window.',
        'goto :eof'
    )
    [System.IO.File]::WriteAllLines($shimPath, $lines, (New-Object System.Text.UTF8Encoding($false)))

    # The USER Path, never the Machine one -- that would need elevation and
    # would affect every account on the box. GetEnvironmentVariable('Path',
    # 'User') returns ONLY the user portion, so rewriting it cannot clobber
    # the machine portion (which $env:Path, being the merged view, would).
    $userPath = [System.Environment]::GetEnvironmentVariable('Path', 'User')
    $entries = @(($userPath -split ';') | Where-Object { $_ })
    if ($entries -notcontains $binDir) {
        $updated = (@($entries) + $binDir) -join ';'
        [System.Environment]::SetEnvironmentVariable('Path', $updated, 'User')
        # A PATH change written to the registry doesn't reach an already-open
        # process, this one included -- so patch the live $env:Path too,
        # otherwise `toaster` would be "not recognized" in the very window
        # that just said it was registered.
        $env:Path = "$env:Path;$binDir"
        Write-Host ""
        Write-Host "Registered the 'toaster' command for your user:" -ForegroundColor Green
        Write-Host "  wrote $shimPath"
        Write-Host "  added $binDir to your user PATH (undo both: toaster uninstall)"
    } else {
        Write-Host "Updated the 'toaster' command at $shimPath" -ForegroundColor Green
    }
    Write-Host "From now on, just run: toaster        (toaster help for the rest)" -ForegroundColor Cyan
    Write-Host "Already-open terminals need reopening before they see it." -ForegroundColor DarkGray
    Write-Host ""
}
...
Install-ToasterShim -RepoRoot $RepoRoot
